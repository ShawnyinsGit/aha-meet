...
2. **fail-closed 权限**：PermissionBroker 超时（默认 120s，可配，需与 server 端 permission 生命周期对齐——Phase 0 验证）或崩溃 → 显式 deny；`end()`/窗口关闭/app 退出 auto-reject 全部 pending。
...
4. **无鉴权 server 缓解组合**（四条，缺一不可）：**仅绑 127.0.0.1**（自写 spawn 后 bind 地址是自由选择，一次疏忽即局域网无鉴权 RCE）+ 随机高端口 + 全代理 + **spawn 注入钉死 config**（`OPENCODE_CONFIG_CONTENT` 设 permission=ask 类，禁 auto-accept——否则 PermissionBroker 被 server 侧配置架空）+ 威胁模型入文档。随机端口对同机攻击者是 obscurity，不能替代前四条。
...
6. **server file API 按 instance directory 隔离**，天然替代 `isPathSafe`——**但隔离强度未验证**（`file.read` 是否拒绝 `../`/绝对路径，SDK 类型不可知），验证前 main 侧仍做 path confinement；验证项列入 Phase 0/1。
...
- **二进制分发（G9，Phase 0 定案）**：`opencode-ai` 入 dependencies（平台 optionalDeps + asarUnpack）；**自写 spawn（估 ~120 行**——含 env 白名单、config 注入、stdout banner 解析、port=0、启动后 exit 监听、进程树 kill），绕开 `createOpencodeServer`（无 binaryPath 注入点、exit handler 仅启动期有效）。与 SDK client 配对：banner 解析得 url 后 `createOpencodeClient({ baseUrl: url, directory: cwd })`（SDK 公开导出，directory 经 header/query 注入），一 `(meetingId, cwd)` 一 client。
- **事件流硬顺序与重连（G3）**：**subscribe-before-create**：subscribe → `session.create` → 注册 sessionID 过滤项 → 首个 prompt（现 adapter 先 create 后订阅，greeting 触发的即时 `permission.updated` 会丢）。断线重连 = **checkpoint-resync 带去重**：重订阅 → buffer 新事件 → 全量拉 `session.messages/todo/diff` → 按 (messageID, partID) 去重（last-write-wins）→ 续播（拉取期间订阅活跃会双计 delta，拉完才订阅会丢窗口期事件，两种裸实现都做不到"断流不丢"）。Phase 0 验证 SSE 是否支持 Last-Event-ID，支持则改断点续传。**SSE 端点选择**（`/global/event` vs per-instance `event.subscribe`，global 流跨 directory 泄漏面）Phase 0 定。
...
- **权限桥（G2）**：新建 provider-neutral PermissionBroker，复用 `auto-approve-policy` 风险分类；值域 UI `allow/deny` → opencode `once/reject`（always 不上 UI）；opencode 工具名 → `SAFE_BUILTIN_TOOLS` 映射表必做；双端可见审批靠 `EventPermissionReplied` 幂等消除。**always 的后续闭环**：Phase 0 spike 加两项验证——server 端 permission 生命周期（有无自身超时）、`always` 答复后同类请求是否还发 `permission.updated`（若不发，broker 对该类工具永久失明）。
...
- **终端（G4）**：`@xterm/xterm` + `addon-fit`（~90KB gzip），命名 `PtyPanel`（避免与 TerminalPanel 冲突）；main 做 WS 代理；PTY 生命周期归 registry，每窗口最多 1 个；安全条款见 §2.2 规则 8。
...
|---|---|---|---|
| 0 | spike：二进制分发、端口分配、server 鉴权、PTY 协议、SSE Last-Event-ID、always 后续行为、server file API 隔离强度 | 干净环境 dev 包起 server；双 OpenCode 参会者同会；鉴权与隔离结论回填 | T0，S0 黑客松（RDK X5） |
